resource "aws_vpc" "primary" {
  ...
  enable_dns_support   = true // this is to enable DNS support in the VPC
  enable_dns_hostnames = true // this is to enable DNS hostnames in the VPC
...
resource "aws_subnet" "secondary_subnet" {
  ...
  tags = {
    Name        = "Primary-subnet-${var.secondary}"
    environment = "demo"
  }
}
...
# Associate route tables with Primary subnet
...
# Associate route tables with Secondary subnet
resource "aws_route_table_association" "asso_secondary" {
  provider       = aws.secondary
